...
contract TWAMMHookIntegrationTest is Test {
    ...
    function test_ClaimOutput_AfterExecution() public {
        _addLiquidity(lp, 50_000 ether, 50_000 ether);
        ...
        uint256 claimableBefore = hook.claimableOutput(orderId);
        TWAMMHook.TWAMMOrder memory order = hook.getOrder(orderId);
        IERC20 outputToken = IERC20(Currency.unwrap(order.tokenOut));
        uint256 aliceOutBefore = outputToken.balanceOf(alice);

        uint256 claimed = hook.claimTWAMMOutput(orderId);

        assertEq(claimed, claimableBefore, "claimed amount should match claimable balance");
        assertEq(hook.claimableOutput(orderId), 0, "claimable should reset to zero");
        assertEq(outputToken.balanceOf(alice), aliceOutBefore + claimableBefore, "user should receive output tokens");
        vm.stopPrank();
    }

    function test_RevertIf_ClaimOutput_NotOrderOwner() public {
        _addLiquidity(lp, 50_000 ether, 50_000 ether);
        ...
        hook.claimTWAMMOutput(orderId);
        vm.stopPrank();
    }

    function test_RevertIf_ClaimOutput_WithoutExecutedChunks() public {
        _addLiquidity(lp, 50_000 ether, 50_000 ether);
        ...
        vm.expectRevert(TWAMMHook.TWAMMHook__NoChunksToExecute.selector);
        hook.claimTWAMMOutput(orderId);
        vm.stopPrank();
    }
