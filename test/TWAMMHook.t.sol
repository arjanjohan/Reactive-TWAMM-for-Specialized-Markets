...
contract TWAMMHookTest is Test {
    ...
    function setUp() public {
        ...
        // Create pool key
        poolKey = PoolKey({
            currency0: Currency.wrap(token0), currency1: Currency.wrap(token1), fee: 3000, tickSpacing: 60, hooks: hook
        });
    ...
    function test_RevertIf_InvalidAmount() public {
        ...
        hook.submitTWAMMOrder(poolKey, 0, 10 minutes, Currency.wrap(address(tokenA)), Currency.wrap(address(tokenB)), 0);
        vm.stopPrank();
    }
    ...
    function test_SubmitOrder_StoresMinOutputPerChunk() public {
        ...
        tokenA.approve(address(hook), 100 ether);
        bytes32 orderId = hook.submitTWAMMOrder(
            poolKey, 100 ether, 10 minutes, Currency.wrap(address(tokenA)), Currency.wrap(address(tokenB)), 77 ether
        );
        vm.stopPrank();
    ...
    function test_GetPoolOrders() public {
        ...
        bytes32 orderId2 = hook.submitTWAMMOrder(
            poolKey, 50 ether, 5 minutes, Currency.wrap(address(tokenA)), Currency.wrap(address(tokenB)), 0
        );
        vm.stopPrank();
